...
class CoverArtIOHandler {
  ...
  cancel(callback) {
    ...
    if (
      cover_url &&
      !Array.from(this._callbacks.values()).includes(cover_url)
    ) {
...
const CoverIcon = GObject.registerClass(
  ...
  class CoverIcon extends St.Icon {
    ...
    update(fallback_icon_name, cover_url) {
      if (
        this._fallback_gicon &&
        !this._fallback_gicon.get_names().includes(fallback_icon_name)
      ) {
  ...
  class ToolTip extends ToolTipBase {
    _init(indicator) {
      ...
      this.pushSignal(
        this.indicator,
        "update-tooltip",
        this.onUpdateToolTip.bind(this)
      );
    }
    ...
    onIndicatorHover(indicator, pspec) {
      if (
        this.indicator.hover &&
        this.text &&
        !this.indicatorMenuIsOpen &&
        !this.focused &&
        !this.visible
      ) {
        this.animatedShow();
...
const Volume = GObject.registerClass(
  ...
  class Volume extends MainItem {
    _init() {
      ...
      this._sliderChangedId = this.pushSignal(
        this._slider,
        "notify::value",
        () => {
    ...
    setProxy(mpris) {
      ...
      this._volumeChangedId = this._mpris.pushSignal(
        this._mpris,
        "notify::volume",
        () => {
          this.value = this._mprisVolume;
    ...
    get _sliderValue() {
      return this._slider.value
        ? Math.max(
            0.0,
            Math.min(Math.round(this._slider.value * 100) / 100, 1.0)
          )
        : 0.0;
    }
    ...
    _init(proxy) {
      ...
      this.pushSignal(this.info, "notify::height", (info) => {
        let size = Math.ceil(
          info.height > 0
            ? info.height
            : 32 / St.ThemeContext.get_for_stage(global.stage).scale_factor
        );
  ...
  class PlayerItem extends TrackItem {
    _init() {
      ...
      this.coverIcon.pushSignal(this.coverIcon, "notify::gicon", () => {
        iconEffect.enabled =
          this.coverIcon.gicon instanceof Gio.BytesIcon ? false : true;
      });
    }
...
class Player extends PopupMenuSection {
  constructor(mpris, trackList, playList, updateIndicator) {
    ...
    this._playerItem.pushSignal(this._playerItem, "notify::hover", () => {
      this._playerItem.closeButton.visible =
        this._playerItem.hover && this._mpris && this._mpris.show_close;
    });

    this._playerItem.pushSignal(
      this._playerItem,
      "key-press-event",
      this._onKeyPressEvent.bind(this)
    );
    this._controls.pushSignal(
      this._controls,
      "key-press-event",
      this._onKeyPressEvent.bind(this)
    );
...
var MprisIndicatorButton = GObject.registerClass(
  ...
  class MprisIndicatorButton extends Button {
    _init() {
      ...
      container.add_child(artistIndicator);
      container.add_child(dash);
      container.add_child(titleIndicator);
        ...
        (proxyHandler, busName, mpris, trackList, playList) => {
          ...
          if (player) {
            ...
            this.menu.addMenuItem(
              new Player(mpris, trackList, playList, updateIndicator)
            );
          }
          updateIndicator();
        }
